...
const ProjectAdd = () => {
  ...
  const addTable = useCallback((e) => {
    if (e.currentTarget.parentElement.querySelector('input').value) {
      if (!isNaN(e.currentTarget.parentElement.querySelector('input').value) && e.currentTarget.parentElement.querySelector('input').value > 0) {
        setTableData([...tableData, { name: e.currentTarget.parentElement.querySelector('input').value, id: numberD += 1 }])
